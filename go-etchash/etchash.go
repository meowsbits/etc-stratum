...
func (l *Light) Verify(block Block) bool {
	...
	*/
	...
	log.Debug("light.verify", "epoch", epoch, "epochLength", epochLength, "blockNum", blockNum)
	...
	if block.MixDigest() != mixDigest {
		log.Debug("unexpected mix digest", "mix digest", block.MixDigest(), "computed mix digest", mixDigest)
		return false
	}
